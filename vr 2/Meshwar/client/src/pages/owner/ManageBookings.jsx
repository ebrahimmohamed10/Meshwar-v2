                ...
                {bookings && bookings.length > 0 && bookings.map((booking, index) => {
                  ...
                  return (
                    <tr key={booking._id || index} className="hover:bg-gray-50/50 transition-colors group">
                      ...
                      {/* Customer Info */}
                      <td 
                        className="py-4 px-6 whitespace-nowrap cursor-pointer hover:bg-gray-100 rounded-lg group transition-colors"
                        onClick={() => {
                          if (booking.user) {
                      ...
                      >
                        <div className="flex items-center">
                          <div className="h-8 w-8 rounded-full bg-gray-100 overflow-hidden flex items-center justify-center text-gray-600 font-bold text-xs mr-3">
                            {booking.user?.image ? (
                          ...
                          <div>
                            <div className="text-sm font-medium text-gray-900 group-hover:text-green-600 transition-colors">{booking.user?.name || 'Unknown User'}</div>
                      ...
                      <td className="py-4 px-6 whitespace-nowrap text-right">
                        <div className="text-sm font-semibold text-gray-900">
                          {typeof booking.price === 'number' ? (booking.price - (booking.priceBreakdown?.taxAmount || 0)).toLocaleString() : '0'} <span className="text-xs text-gray-500 font-normal">{currency}</span>
                        </div>
                      </td>
